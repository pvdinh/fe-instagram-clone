...
import {
    beginFollowing, deleteUserHistory,
...
function* saga_deleteUserHistory(action) {
    try {
        const response = yield call(deleteUserHistory,action.id)
...
function* listen() {
    ...
    yield takeEvery(homeActions.type.DELETE_USER_HISTORY, saga_deleteUserHistory)
}
