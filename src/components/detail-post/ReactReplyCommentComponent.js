...
function ReactReplyCommentComponent(props) {
    ...
    const likeReplyComment = () =>{
        props.likeReplyComment(props.comment.id,(data)=>{
    ...
    const unLikeReplyComment = () =>{
        props.unLikeReplyComment(props.comment.id,(data)=>{
            if(data.data === 'success'){
                setLike(false)
            }
        })
            ...
            <div className="wrap-like" style={{cursor:"pointer"}}>
                {
                    like ?
                        <svg className="_8-yf5 " height="13" fill="var(--like)"
                             viewBox="0 0 48 48" width="13" onClick={()=>{unLikeReplyComment();setLike(!like)}}>
                            <path
                        ...
                        <div className="post__button black">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                 xmlns="http://www.w3.org/2000/svg" onClick={()=>{likeReplyComment();setLike(!like)}}>
                                <path
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        likeReplyComment: (idComment,callback) =>{
          dispatch(comment.action.likeReplyComment(idComment,callback))
        },
        unLikeReplyComment: (idComment,callback) =>{
          dispatch(comment.action.unLikeReplyComment(idComment,callback))
        },
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(ReactReplyCommentComponent)